...
const FOLLOWUP_SCRIPTS = {
  'followup_1': {
    ...
    content: (businessName) => `Hi ${businessName} Team,

Just checking in to see if you had a chance to think about the website we discussed.

I know you're busy running your business, so I'll keep this brief — having a website could bring in new customers while you focus on what you do best.

Would you be open to a quick 15-minute call to chat about it?

Best,
Wandile`
  },
  'followup_2': {
    label: '📧 Follow-up 2 (5 days)',
    content: (businessName) => `Hi again,

I wanted to share a quick example of a website I built for a similar business in your area — they went from no online presence to getting 10+ customer inquiries in their first week.

It's really about making sure people can find you when they search for your services.

Can I send you a concept for ${businessName}? No pressure at all.

Wandile`
  },
  'followup_3': {
    label: '📞 Call Follow-up',
    content: (businessName) => `📞 FOLLOW-UP CALL

"Hi, this is Wandile from Jigsaw AI — we spoke the other day about a website for ${businessName}."

"Just wanted to follow up and see if you had any questions or if you'd like me to send over a concept."

"Happy to jump on a quick call this week if that works better for you."
`
  }
};
...
const REPLY_HANDLERS = {
  'not_interested': {
    ...
    content: (businessName) => `Hi there,

Thanks for getting back to me.

I completely understand — this might not be the right time for ${businessName}.

Just so you know, we're here if you ever change your mind. A website can make a real difference in attracting new customers.
...
Wandile`
  ...
  'send_info': {
    ...
    content: (businessName) => `Hi,

Thanks for your interest — I'm happy to share more details.

We build professional websites starting from R2,500/month, including:
- Custom mobile-friendly design
- SEO optimization
- Contact forms and lead capture
- Hosting and support

Would you like me to send a proposal tailored to ${businessName}?
...
That's great to hear! I'd love to see what you've built.

Sometimes businesses have a website but it's not optimized for generating leads. We might be able to help improve your online presence.

Would you be open to a quick review of your current website?
...
"Are you looking for something simple, or are you open to a full website with lead generation features?"
`
  }
};
...
function StatCard({ icon: Icon, label, value, color = 'text-white' }) {
  return (
    <div className="flex items-center gap-3 bg-[#131C31] rounded-xl px-4 py-2.5">
      <div className={`p-2 rounded-lg bg-[#18233D] ${color}`}>
        <Icon size={16} />
      </div>
      <div>
...
function ScriptPanel({ title, icon: Icon, content, color }) {
...
export default function CampaignDashboard() {
  ...
  async function loadCampaignData() {
    ...
    setLeads(leadsData || []);
    if (leadsData?.length && !selectedId) setSelectedId(leadsData[0].id);
    setLoading(false);
  }
...
• No lead generation = relying only on word of mouth`
  ...
  // Stats based on campaign leads only
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#090D18]">
        <div className="text-slate-400 text-lg">Loading campaign...</div>
      </div>
    );
  }
    ...
    <div className="flex h-screen bg-[#090D18] text-slate-200 overflow-hidden">
      ...
      <div className="w-56 bg-[#0D1424] border-r border-[#18233D] flex flex-col p-5 shrink-0">
        ...
        <nav className="flex flex-col gap-0.5 text-sm">
          <Link href={`/campaigns/${campaignId}/dashboard`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-[#5B7CFA]/10 text-[#5B7CFA] text-sm font-medium">
            <LayoutDashboard size={17} /> Dashboard
          </Link>
          <Link href={`/campaigns/${campaignId}/leads`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#18233D] transition-colors text-sm">
            <Users size={17} /> Leads
          </Link>
          <Link href={`/campaigns/${campaignId}/pipeline`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#18233D] transition-colors text-sm">
            <Kanban size={17} /> Pipeline
          </Link>
          <Link href={`/campaigns/${campaignId}/outreach`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#18233D] transition-colors text-sm">
            <Mail size={17} /> Outreach
          </Link>
          <Link href="/calendar" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#18233D] transition-colors text-sm">
            <CalendarIcon size={17} /> Calendar
          </Link>
          <Link href="/campaigns" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-[#18233D] transition-colors text-sm mt-4 border-t border-[#18233D] pt-4">
            <ArrowLeft size={17} /> All Campaigns
          </Link>
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-16 border-b border-[#18233D] px-8 flex items-center justify-between shrink-0 bg-[#090D18]">
          <div className="flex items-center gap-4">
            <Link href="/campaigns" className="text-slate-400 hover:text-white transition-colors flex items-center gap-2">
              <ArrowLeft size={18} /> Campaigns
            </Link>
            <h1 className="text-xl font-bold text-white">{campaign.name}</h1>
            <span className="text-sm text-slate-500">{campaign.industry || 'No industry'}</span>
            {isArchived && (
        ...
        <div className="border-b border-[#18233D] bg-[#0D1424] px-8 py-4">
          <div className="flex flex-wrap gap-8">
            ...
            <div>
              ...
              <div className="text-2xl font-bold text-rose-400">{hotCount}</div>
            </div>
            <div>
              <div className="text-xs text-slate-400">🟠 Warm</div>
              <div className="text-2xl font-bold text-orange-400">
                {leads.filter(l => l.eval_opportunity_level === '🟠 Warm' && (!l.pipeline_status || l.pipeline_status === 'new' || l.pipeline_status === 'analyzed' || l.pipeline_status === 'drafted')).length}
              </div>
            </div>
            <div>
              <div className="text-xs text-slate-400">🏆 Won</div>
              <div className="text-2xl font-bold text-emerald-400">
                {leads.filter(l => l.pipeline_status === 'won').length}
              </div>
            </div>
            <div>
            ...
            <div>
              <div className="text-xs text-slate-400">❌ No Reply</div>
              <div className="text-2xl font-bold text-red-400">{noReplyCount}</div>
            </div>
          </div>
        </div>
                  ...
                  <div className="bg-[#0D1424] border border-[#18233D] rounded-2xl p-6">
                    ...
                    {selected.website ? (
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-2">This is a great opportunity.</p>
                        {selected?.eval_recommended_solution && (
                          <div className="mt-4 pt-4 border-t border-[#18233D]">
                            <div className="text-sm font-medium text-white">Recommended Package</div>
                            <div className="text-sm text-[#5B7CFA] mt-1">{selected.eval_recommended_solution.package_type}</div>
